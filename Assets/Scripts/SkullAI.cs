...
public class SkullAI : MonoBehaviour
{
    ...
    //public bool isGrounded; // skull has no legs :( but it floats in the air :)
    ...
    void Start()
    {
        ...
        _finalDamage = 0f;
        _weaponDamage = gm.skullAttributes.GetBaseDamage(gm.skullLevel);
        ...
        _staminaTotalRecoveryMultiplier = 1 + _staminaRecoveryMultiplier * gm.skullAttributes.GetSta(gm.skullLevel);
    ...
    public void MoveRight()
    {
        ...
        rb.AddForce(Vector3.right * (agilityMultiplier * moveMultiplier * (1.0f + (0.1f * gm.skullAttributes.GetAgi(gm.skullLevel)))));
        state = (int)State.ADVANCE;
        wait = false;
        once = false;
    }
    ...
    public void MoveLeft()
    {
        ...
        if (staminaBar != null) { staminaBar.SendMessage("TakeDamage", _staminaMoveTotal); }
        rb.AddForce(Vector3.left * (agilityMultiplier * moveMultiplier * (1.0f + (0.1f * gm.skullAttributes.GetAgi(gm.skullLevel)))));
        state = (int)State.ADVANCE;
        wait = false;
        once = false;
    }
    ...
    public void Plunge() //damage dealing done in Airborne(), so it deals damage when grounded
    {
        ...
        if (healthBar != null)
        {
            float hitChance = (gm.skullAttributes.GetAtk(gm.skullLevel) * attackType * attackMultiplier) / ((gm.skullAttributes.GetAtk(gm.skullLevel) + PlayerAttributes.pDef) + playerDefenseMultiplier);
            float hitRandom = Random.Range(0.00001f, 10.0000000000000000001f) / 10;
            _finalDamage = 0;
            _finalDamage = (gm.skullAttributes.GetStr(gm.skullLevel) * 2 + _weaponDamage) * 0.7f;
    ...
    public void HeavyAttack()
    {
        ...
        if (staminaBar != null)
        {
            float totalCost = (_staminaAttackBase + gm.skullAttributes.GetStr(gm.skullLevel) * _staminaHeavyAttackMultiplier);
        ...
        if (healthBar != null)
        {
            ...
            _finalDamage = 0;
            _finalDamage = (gm.skullAttributes.GetStr(gm.skullLevel) * 2 + _weaponDamage) * 1.5f;
            ...
            {
                StartCoroutine(player.Dodge(0.8f));
                Debug.Log("AI missed");
                state = (int)State.ADVANCE;
                wait = false;
                once = false;
    ...
    public void MediumAttack()
    {
        ...
        if (staminaBar != null)
        {
            float totalCost = (_staminaAttackBase + gm.skullAttributes.GetStr(gm.skullLevel) * _staminaMediumAttackMultiplier);
            staminaBar.SendMessage("TakeDamage", totalCost);
        }
        if (healthBar != null)
        {
            ...
            _finalDamage = 0;
            _finalDamage = (gm.skullAttributes.GetStr(gm.skullLevel) * 2 + _weaponDamage) * 1f;
    ...
    public void QuickAttack()
    {
        ...
        if (staminaBar != null)
        {
            float totalCost = (_staminaAttackBase + gm.skullAttributes.GetStr(gm.skullLevel) * _staminaQuickAttackMultiplier);
            staminaBar.SendMessage("TakeDamage", 13);
        }
        if (healthBar != null)
        {
            enemyAnimator.SetTrigger("QuickAttack");
            float hitChance = (gm.skullAttributes.GetAtk(gm.skullLevel) * attackType * attackMultiplier) / ((gm.skullAttributes.GetAtk(gm.skullLevel) + PlayerAttributes.pDef) + playerDefenseMultiplier);
            float hitRandom = Random.Range(0.00001f, 10.0000000000000000001f) / 10;
            _finalDamage = 0;
            _finalDamage = (gm.skullAttributes.GetStr(gm.skullLevel) * 2 + _weaponDamage) * 0.7f;
