...
public class LeshenAI : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        _finalDamage = 0f;
        _weaponDamage = gm.leshenAttributes.GetBaseDamage(gm.leshenLevel);
        ...
        _staminaTotalRecoveryMultiplier = 1 + _staminaRecoveryMultiplier * gm.leshenAttributes.GetSta(gm.leshenLevel);
    ...
    public void HeavyAttack()
    {
        ...
        if (staminaBar != null)
        {
            float totalCost = (_staminaAttackBase + gm.leshenAttributes.GetStr(gm.leshenLevel) * _staminaHeavyAttackMultiplier);
        ...
        if (healthBar != null)
        {
            ...
            float hitChance = (gm.leshenAttributes.GetAtk(gm.leshenLevel) * attackType * attackMultiplier) / ((gm.leshenAttributes.GetAtk(gm.leshenLevel) + PlayerAttributes.pDef) + playerDefenseMultiplier);
            float hitRandom = Random.Range(0.00001f, 10.00000000001f) / 10;
            _finalDamage = 0;
            _finalDamage = (gm.leshenAttributes.GetStr(gm.leshenLevel) * 2 + _weaponDamage) * 1.5f;
    ...
    public void MediumAttack()
    {
        ...
        if (staminaBar != null)
        {
            float totalCost = (_staminaAttackBase + gm.leshenAttributes.GetStr(gm.leshenLevel) * _staminaMediumAttackMultiplier);
            staminaBar.SendMessage("TakeDamage", totalCost);
        }
        if (healthBar != null)
        {
            ...
            _finalDamage = 0;
            _finalDamage = (gm.leshenAttributes.GetStr(gm.leshenLevel) * 2 + _weaponDamage) * 1f;
            ...
            if (hitRandom < hitChance)
            {
                ...
                {
                    _finalDamage -= GameManager.instance._armorPlayer;
                    armorBar.SendMessage("TakeDamage", Mathf.Max(0, 100000));
                    healthBar.SendMessage("TakeDamage", _finalDamage);
                    GameManager.instance._armorPlayer = 0f;
                    GameManager.instance._healthPlayer -= _finalDamage;
                }
    ...
    public void QuickAttack()
    {
        ...
        if (staminaBar != null)
        {
            float totalCost = (_staminaAttackBase + gm.leshenAttributes.GetStr(gm.leshenLevel) * _staminaQuickAttackMultiplier);
        ...
        if (healthBar != null)
        {
            ...
            _finalDamage = 0;
            _finalDamage = (gm.leshenAttributes.GetStr(gm.leshenLevel) * 2 + _weaponDamage) * 0.7f;
    ...
    IEnumerator WalkForward()
    {
        ...
        float waitTime = 1f + waitScalar * gm.leshenAttributes.GetAgi(gm.leshenLevel);
        yield return new WaitForSeconds(waitTime);
    ...
    {
        enemyAnimator.SetBool("Idle", false);
        enemyAnimator.SetBool("WalkBackwards", true);
        float waitTime = 1f + waitScalar * gm.leshenAttributes.GetAgi(gm.leshenLevel);
        yield return new WaitForSeconds(waitTime);
        enemyAnimator.SetBool("Idle", true);
        enemyAnimator.SetBool("WalkBackwards", false);
